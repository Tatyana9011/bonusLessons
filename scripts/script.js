document.addEventListener('DOMContentLoaded', () => {
  ...
  const citiesList = JSON.parse(localStorage.getItem('city')) ? JSON.parse(localStorage.getItem('city')) : [];
  const countryList = JSON.parse(localStorage.getItem('country')) ? JSON.parse(localStorage.getItem('country')) : [];
  let language = '';

  const saveCookie = () => {
    language = document.cookie ? document.cookie : prompt('Выберите предпочтительный язык', "RU, EN или DE");
    const expires = new Date(2021, 5, 25);
    document.cookie = `${encodeURI(language)}; expires=` + expires.toGMTString();
  };
  ...
  const saveDataJSON = (name, dataObject) => {
  ...
  const getData = data => {
    ...
    saveDataJSON('city', citiesList);
    saveDataJSON('country', countryList);
  ...
  class Cities {
    ...
    selectAutocomplete(event) {
      ...
      if (result.length) {
        ...
        element2.set('zero-key', this.createCountryList(`${event.target.value} в списке не найдено`,
          ' ', classNameCity));
      }
  ...
  const cityResult = select => {
    ...
    dropdownListLine.forEach(elem => {
      elem.addEventListener('click', event => {
        ...
        const target = event.target;
        selectCities.focus();
        selectCities.value = target.textContent.trim().match(/^.*[^\d]/);
  ...
  const startClickSelect = () => {
    ...
    dropdownListTotalLine.forEach(elem => {
      elem.addEventListener('click', event => {
        ...
        recursDefault();
        selectCities.value = elem.textContent.trim().match(/^.*[^\d]/);
        selectCities.focus();
        dropdownListsListDefault.style.display = 'none';
